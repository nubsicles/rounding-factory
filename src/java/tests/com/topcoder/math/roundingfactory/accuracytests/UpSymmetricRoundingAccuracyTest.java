...
/**
 * Accuracy tests for UpSymmetricRounding.
 *
 * Truncate everything after the accuracy digit, and round a such that the value of the number increases(The absolute
 * value decrease); if the input is a "round number", return itself with proper precision.That is, if the input given
 * would result from a call to round(), then do nothing except the truncation.
 *
 * Example:
 ...
 * @version 1.0
 */
public class UpSymmetricRoundingAccuracyTest extends TestCase implements RoundingAccuracyTestConstants {
    /** The UpSymmetricRounding used for testing. */
    private Algorithm rounding = null;

    /** Create a UpSymmetricRounding instance.*/
    public void setUp() {
        rounding = new UpSymmetricRounding();
    }
