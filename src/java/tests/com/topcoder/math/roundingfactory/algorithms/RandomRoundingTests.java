...
/**
 * <p>Tests the RandomRounding class.</p>
 *
 * @author TCSDEVELOPER
 *
 * @copyright (c) 2005, Topcoder, Inc. All Rights Reserved.
 ...
 * @version 1.0
 */
public class RandomRoundingTests extends TestCase {
    ...
    /**
     * A RandomRounding instance for tests.
     */
     ...
     */
    public void testRandomRounding1() {
     ...
     */
    public void testRandomRounding4() {
        try {
            new RandomRounding(3, 0);
            fail("IllegalArgumentException should be thrown");
        } catch (IllegalArgumentException e) {
            // success
        }
     ...
     * Invalid comparison digits, IllegalArgumentException should be thrown.
     */
    public void testRandomRounding5() {
        try {
            new RandomRounding(3, 10);
            fail("IllegalArgumentException should be thrown");
        } catch (IllegalArgumentException e) {
            // success
        }
    ...
    public void testRound7() {
        ...
        // the result may be -123.45 or -123.44
        String result = algorithm.round("-123.445", 2, 5);
        assertTrue("result should be -123.45 or -123.44", "-123.45".equals(result) || "-123.44".equals(result));

        // the result may be -123.45 or -123.44
        result = algorithm.round("123.445", 2, 5);
        assertTrue("result should be 123.45 or 123.44", "123.45".equals(result) || "123.44".equals(result));
        ...
        assertEquals("result should be 10", "10", algorithm.round("10.4", 0, 5));
    ...
    public void testRound12() {
        ...
        // round some general numbers.
        assertEquals("result should be 123.45", "123.45", algorithm.round("+123.451"));
        assertEquals("result should be -123.45", "-123.45", algorithm.round("-123.451"));
        assertEquals("result should be 123.45", "123.45", algorithm.round("+123.449"));
        assertEquals("result should be -123.45", "-123.45", algorithm.round("-123.449"));

        // the result may be -123.45 or -123.44
        String result = algorithm.round("-123.445");
        assertTrue("result should be -123.45 or -123.44", "-123.45".equals(result) || "-123.44".equals(result));

        // the result may be -123.45 or -123.44
        result = algorithm.round("123.445");
        assertTrue("result should be 123.45 or 123.44", "123.45".equals(result) || "123.44".equals(result));
    ...
    public void testRoundDouble4() {
        ...
        // round some general numbers.
        assertEquals("result should be 123.45", 123.45, algorithm.roundDouble(+123.451, 2, 5), DELTA);
        assertEquals("result should be -123.45", -123.45, algorithm.roundDouble(-123.451, 2, 5), DELTA);
        assertEquals("result should be 123.45", 123.45, algorithm.roundDouble(+123.449, 2, 5), DELTA);
        assertEquals("result should be -123.45", -123.45, algorithm.roundDouble(-123.449, 2, 5), DELTA);
